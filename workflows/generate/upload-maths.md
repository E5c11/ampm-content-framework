...
5. **Vocab dump**: skip — no Maths curriculum collection yet; validator runs without
   `--curriculum` (profile).
6. **Upload script**: `add-maths-<year>-<paper>-q<N>.js`; collections
   `maths_videos`/`maths_questions`. Validate (HARD STOP) → dev only → **commit**
   (`[Data] Add maths <year> <paper> Q<N> video and questions`).
