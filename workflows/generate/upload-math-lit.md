...
4. **Questions** (pipeline Phase 3): 2–4 questions, fresh scenarios (`DESIGN-UNI-01`),
   ...
   vocabulary reuse-before-creating against `math_lit_curriculum`/`math_lit_skills`
   (`PIPE-08`).
5. **Vocab dump** (pipeline Phase 3.5): `--project dev`, no `--subject` flag (math_lit
   is the default).
6. **Upload script** (pipeline Phase 4): `add-<year>-<paper>-q<N>.js`; collections
   `math_videos`/`math_questions`; ai_explanation per exam sub-question with real
   `marks`. Validate with `--curriculum` — HARD STOP (`PIPE-10`) — then run against dev
   only. **Commit** (`[Data] Add math_lit <year> <paper> Q<N> video and questions`).
7. **Verify** (pipeline Phase 5 checklist), plus: `paper_path`
   `"dbe/math_lit/YYYY/<paper>"`; annexure completeness.
